...
def train_crop_prediction_model():
    ...
    # Load the dataset
    data = pd.read_csv('DBMYes/CROPP.csv')
...
def main():
    ...
    # Read the CSV files
    soil_data_df = pd.read_csv('DBMYes\soil_data1.csv')
    coordinates_df = pd.read_csv('DBMYes\district_coordinates.csv')
    ...
    if location:
        ...
        if district:
            ...
            if not soil_data.empty:
                ...
                # top_three_probs = [probabilities[0][i] for i in top_three_indices]

                print("The top three recommended crops for the input values are:")
                for crop in top_three_crops:
                    print(crop)
                    # print(f"{crop} with probability {prob:.2f}")
            else:
